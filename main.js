...
function similarity(token_1, token_2) {
    const frequency_1 = character_frequency(token_1);
    const frequency_2 = character_frequency(token_2);
    
    const all_characters = new Set([...Object.keys(frequency_1), ...Object.keys(frequency_2)]);
    
    let difference = 0;
    for (const char of all_characters) {
        const count_1 = frequency_1[char] || 0;
        const count_2 = frequency_2[char] || 0;
...
function fuzzy_match(tokens_1, tokens_2) {
    ...
    const base_score = matched_tokens.size / Math.max(tokens_1_size, tokens_2_size);
    const final_score = Math.max(0, base_score * 100 - penalties);

    return Math.round(final_score * 100) / 100;
}
